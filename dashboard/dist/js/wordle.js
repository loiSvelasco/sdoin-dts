...
function checkHardInput (word, ultra = false, attemptsMade = state.wordsTried) {
  for (let attemptMade of attemptsMade) {
    ...
    for (let index = 0; index < wordToGuess.length; index++) {
      ...
      if (attemptMade[index] === wordToGuess[index]) {
        ...
        (
          ...
          (
            state.lettersTried[currentGuess[index]] === 'correct' &&
            attemptMade[index] === currentGuess[index] &&
...
function getPossibleWordsLeft () {
  const dictionary = dictionaries[state.config.wordLength]
  const attemptsMade = isGameOver() ? state.wordsTried.slice(0, -1) : undefined
  return [
    dictionary.wordsGuessable,
    dictionary.wordsAcceptable
  ]
    .flatMap(s => Array.from(s))
    .filter(w => !checkHardInput(w, state.config.ultraHardMode, attemptsMade))
}
...
function saveState () {
  ...
  let userState = window.localStorage.getItem('state');
  $.ajax({
    dataType: "json",
    method : "POST",
    // url: '../requests/saveWordle.php', // if using iframe
    url: 'requests/saveWordle.php', // if html is loaded directly
    data: {userWordle:userState},
    success: function(){},
    error: function(){
      alert('An error occured while saving, please refresh the browser.');
    }
  });
}
